...
locals {
  ...
  service_name              = "chs-kafka-api-java"
  container_port            = "8080"
  eric_port                 = "10000"
  docker_repo               = "chs-kafka-api-java"
  kms_alias                 = "alias/${var.aws_profile}/environment-services-kms"
  lb_listener_rule_priority = 40
  lb_listener_paths         = ["/private/filing-processed","/message-send","/private/resource-changed","/send-email", "/private/filing-processed"]
  healthcheck_path          = "/healthcheck"
  healthcheck_matcher       = "200"
  vpc_name                  = local.stack_secrets["vpc_name"]
  app_environment_filename  = "chs-kafka-api-java.env"
